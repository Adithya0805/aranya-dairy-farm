...
export default function ChatAssistantWidget() {
  ...
  // Listen to global open-farm-ai event from QuickActionTileRow or other links
  useEffect(() => {
    const handleOpen = () => {
      setIsOpen(true);
    };
    window.addEventListener('open-farm-ai', handleOpen);
    return () => window.removeEventListener('open-farm-ai', handleOpen);
  }, []);
    ...
    <>
      ...
      <div className="fixed bottom-[8.5rem] right-4 sm:bottom-22 sm:right-6 z-40">
        <button
          ...
          className="relative flex items-center justify-center gap-2 bg-[#15321E] hover:bg-[#1C3E25] active:scale-95 text-white p-3 sm:px-4 sm:py-3.5 rounded-full shadow-xl hover:shadow-2xl border-2 border-[#E58A13]/80 transition-all duration-200 cursor-pointer min-h-[48px] min-w-[48px] touch-manipulation group animate-floating-pulse"
        >
          {isOpen ? (
      ...
      {isOpen && (
        <div
          ...
          className={`
            ...
            /* Mobile layout: inset from edges, comfortably above bottom nav */
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Contextual WhatsApp Handoff CTA inside message if intent is detected */}
